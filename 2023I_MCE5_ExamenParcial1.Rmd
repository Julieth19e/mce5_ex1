...
**Respuesta
$$Ha$$La facilitación interviene en la supervivencia de la especies de plantas en la etapa madura.
$$Ho$$La fase adulta de las plantas depende de la facilitación para su supervivencia**

*Explicación: La facilitación es un mecanismo que ayuda a que la diversidad de especies raras localmente no pueden ser extiguidas en etapas juveniles ademas de bajar su nivel de estres en el ambiente.Por lo que se puede deducir que las plantas adultas no pueden depender de la facilitacion que vaya haver en su territorio.
...
```{r crd_anfibios}
sitios = c("Loreto", "Francisco de Orellana", "Joya de los Sachas" "Limoncocha")
```

**Respuesta:**
Se escogio estos 4 sitios que no se encuentran separados a más de 20 km para el previo estudio y el porque se escogio este parámetro es para que sea mínimos los cambios de las condiciones climáticas entre ellas.


## Variables
¿Cuáles son sus variables (dependiente(s) e independiente(s))? Explique cada una de las variables

**Variable independiente: yeso **
**Variable dependiente: plantas**
...
```{r}
jabali <- read.table("s3_boar.txt", header = T)
par(mfrow=c(1,2))

boxplot(jabali$LengthCT ~ jabali$Tb, ylab = "Longitu cabeza- torax (cm)", xlab = "Presencia de Tuberculosis")

plot(jabali$LengthCT, jabali$Tb, ylab = "Longitu cabeza- torax (cm)", xlab = "Presencia de Tuberculosis")
reg <- lm(jabali$LengthCT ~ jabali$Tb)
abline(reg)
```
